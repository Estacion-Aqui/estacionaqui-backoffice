...
import { getAllStaticData } from "variables/apis.js";

const Header = () => {
  const [result, setResult] = useState({});

  useEffect(() => {
    getAllStaticData().then(function (result) {
      setResult(result);
    });
  });
  return (
                ...
                <Card className="card-stats mb-4 mb-xl-0">
                  <CardBody>
                    ...
                    <p className="mt-3 mb-0 text-muted text-sm">
                      {result.userData.direction ? (
                        <span className="text-success mr-2">
                          <i className="fa fa-arrow-danger" />{" "}
                          {result.userData.percent}%
                        </span>
                      ) : (
                        <span className="text-success mr-2">
